...
return new class extends Migration
{
    ...
    {
        // 1. New Payroll Products Table
        if (!Schema::hasTable('payroll_products')) {
    Schema::create('payroll_products', function (Blueprint $table) {
        $table->id();
        $table->string('product_name', 150);
        $table->integer('target_weekday')->default(0);  // Target Mon-Fri
        $table->integer('target_saturday')->default(0); // Target Saturday
        $table->decimal('rate_above', 10, 2)->default(0.00); // Rate - Above target
        $table->decimal('rate_below', 10, 2)->default(0.00); // Rate - Below target
        ...
        // 3. Timecards Table (Daily Attendance)
        ...
        // 4. Timecard Products Table (Daily Production Quantity)
        if (!Schema::hasTable('timecard_products')) {
            Schema::create('timecard_products', function (Blueprint $table) {
                ...
                $table->foreignId('product_id')->constrained('payroll_products')->onDelete('restrict');
        ...
        // 5. Payroll Summaries Table (Monthly Pay Total)
